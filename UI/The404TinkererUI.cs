...
namespace MoTools.UI
{
	// This class represents the UIState for our The404Tinkerer Awesomeify chat function. It is similar to the Goblin Tinkerer's Reforge function, except it only gives Awesome and ReallyAwesome prefixes. 
	internal class The404TinkererUI : UIState
	{
		...
		// OnDeactivate is called when the UserInterface switches to a different state. In this mod, we switch between no state (null) and this state (The404TinkererUI).
		...
		public override void OnDeactivate() {
			...
			// Note that in The404Tinkerer we call .SetState(new UI.The404TinkererUI());, thereby creating a new instance of this UIState each time. 
		...
		public override void Update(GameTime gameTime) {
			...
			if (Main.LocalPlayer.talkNPC == -1 || Main.npc[Main.LocalPlayer.talkNPC].type != NPCType<The404Tinkerer>()) {
				...
				GetInstance<MoTools>().The404TinkererUserInterface.SetState(null);
			}
		}
		...
		protected override void DrawSelf(SpriteBatch spriteBatch) {
			...
			const int slotY = 270;
			if (!_vanillaItemSlot.Item.IsAir) {
				int speedOfLightPrice = Item.buyPrice(0, 1, 0, 0);

				string costText = Language.GetTextValue("LegacyInterface.46") + ": ";
				string coinsText = "";
				int[] coins = Utils.CoinsSplit(speedOfLightPrice);
				if (coins[3] > 0) {
				...
				if (hoveringOverReforgeButton) {
					...
					if (Main.mouseLeftRelease && Main.mouseLeft && Main.LocalPlayer.CanBuyItem(speedOfLightPrice, -1) && ItemLoader.PreReforge(_vanillaItemSlot.Item)) {
						Main.LocalPlayer.BuyItem(speedOfLightPrice, -1);
						...
						if (Main.rand.NextBool(10)) {
							reforgeItem.Prefix(GetInstance<MoTools>().PrefixType("SpeedOfLight"));
						}
						else {
							reforgeItem.Prefix(GetInstance<MoTools>().PrefixType("Colossal"));
						}
				...
				string message = "Place an item here to 404 Reforge";
