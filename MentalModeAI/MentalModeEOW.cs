...
namespace MoTools.MentalModeAI
{
...

	{
		...
		public override bool PreAI(NPC npc)
		{
			...
			if (npc.type >= NPCID.EaterofWorldsHead && npc.type <= NPCID.EaterofWorldsTail && MoToolsWorld.MentalMode)
			{

				if (Main.netMode != NetmodeID.MultiplayerClient)
				{
					if (npc.type == NPCID.EaterofWorldsBody && ((double)(npc.position.Y / 16f) < Main.worldSurface || MoToolsWorld.MentalMode))
					{
