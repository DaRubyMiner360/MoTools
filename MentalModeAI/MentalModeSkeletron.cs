...
namespace MoTools.MentalModeAI
{
...

    {
        ...
        {
            if (npc.type == NPCID.SkeletronHead && MoToolsWorld.MentalMode)
            {
                int DefenseInc = 0;
                for (int i = 0; i < 200; i++)
                {
            ...
            //// Hands
            if (npc.aiStyle == 12 && MoToolsWorld.MentalMode)
            {
            ...
            if ((npc.type == NPCID.SkeletronHead || npc.aiStyle == 12) && MoToolsWorld.MentalMode)
            { return false; }
            else { return true; }
        }
    }
