...
{
    public class SteamHouses
    {
        private static int numRooms = 0;
        private static int direction = 0;
        private static int roomHeight = 0;
        private static int blockType = ModContent.TileType<Tiles.SteamBrick>();
        private static int platformType = TileID.Platforms;
        private static int wallType = ModContent.WallType<SteamBrickWall>();
        private static int x = 0;
        private static int y = 0;

        private static List<int> nonOverrideBlocks = new List<int> { blockType, platformType, ModContent.TileType<SteamDoorClosed>(), ModContent.TileType<LavaGemLanternTile>(), ModContent.TileType<LavaGemLampTile>() , ModContent.TileType<LavaGemLanternTile>(),
            ModContent.TileType<SteamTableTile>(), ModContent.TileType<SteamDresserTile>(), ModContent.TileType<SteamBedTile>(), ModContent.TileType<SteamSinkTile>(), ModContent.TileType<SteamWorkbenchTile>() };
        ...
        private static void BuildDoor(int height, int width)
        {
            if(direction == 1)
            {
                if(TileCheckSafe(x + width - 1, y + height - 2))
                {
                    ...
                    WorldGen.PlaceObject(x + width - 1, y + height - 3, ModContent.TileType<SteamDoorClosed>());
                }
            }
