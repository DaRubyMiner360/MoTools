...
{
	public class The404Bow : ModItem
	{
		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("404 Bow");
			Tooltip.SetDefault("A more advanced version of the P90, but as a bow!");
		}
		...
		public override bool Shoot(Player player, ref Vector2 position, ref float speedX, ref float speedY, ref int type, ref int damage, ref float knockBack)
		{
			...
			{
				type = ProjectileType<Projectiles.Ranged.The404Arrow>(); // or ProjectileID.FireArrow;
			}
			return true; // return true to allow tmodloader to call Projectile.NewProjectile as normal
			float numberProjectiles = 7 + Main.rand.Next(7); // 3, 4, or 5 shots
		...
		/*public override bool Shoot(Player player, ref Vector2 position, ref float speedX, ref float speedY, ref int type, ref int damage, ref float knockBack)
		{
			float numberProjectiles = 5 + Main.rand.Next(5); // 3, 4, or 5 shots
			float rotation = MathHelper.ToRadians(45);
			position += Vector2.Normalize(new Vector2(speedX, speedY)) * 45f;
			for (int i = 0; i < numberProjectiles; i++)
			{
