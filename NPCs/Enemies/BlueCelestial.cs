...
{
	public class BlueCelestial : Hover
	{
		public BlueCelestial() {
			speedY = 1f;
			accelerationY = 0.1f;
		}
		...
		public override void SetDefaults() {
			...
			banner = npc.type;
			bannerItem = ItemType<BlueCelestialBanner>();
		}
			...
			if (ai < 0f) {
				...
				if (ai == -1f)
				{
					for (int k = 0; k < 255; k++)
					{
						...
						{
							target.AddBuff(BuffID.WaterWalking, 240, true);
							target.AddBuff(BuffID.WaterCandle, 240, true);
							target.AddBuff(BuffID.Gills, 240, true);
							target.AddBuff(BuffID.ObsidianSkin, 240, true);
						}
					}
				}
				if (ai == -61f) {
		...
		{
			Item.NewItem(npc.getRect(), mod.ItemType("BlueCelestialShard"), Main.rand.Next(15, 18));
		...
		public override void OnHitPlayer(Player player, int damage, bool crit) {
			if (Main.rand.NextBool(3)) {
				player.AddBuff(BuffID.WaterWalking, 240, true);
				player.AddBuff(BuffID.WaterCandle, 240, true);
				player.AddBuff(BuffID.Gills, 240, true);
				player.AddBuff(BuffID.ObsidianSkin, 240, true);
			}
		}
