...
{
	public class YellowCelestial : Hover
	{
		public YellowCelestial() {
			speedY = 1f;
			accelerationY = 0.1f;
		}
		...
		public override void SetDefaults() {
			...
			banner = npc.type;
			bannerItem = ItemType<YellowCelestialBanner>();
		}
			...
			if (ai < 0f) {
				...
				if (ai == -1f)
				{
					for (int k = 0; k < 255; k++)
					{
						...
						{
							target.AddBuff(BuffID.Spelunker, 240, true);
							target.AddBuff(BuffID.Shine, 240, true);
						}
					}
				}
				if (ai == -61f) {
		...
		{
			Item.NewItem(npc.getRect(), mod.ItemType("YellowCelestialShard"), Main.rand.Next(15, 18));
		...
		public override void OnHitPlayer(Player player, int damage, bool crit) {
			if (Main.rand.NextBool(3)) {
				player.AddBuff(BuffID.Spelunker, 240, true);
				player.AddBuff(BuffID.Shine, 240, true);
			}
		}
