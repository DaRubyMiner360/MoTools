...
{
	public class WhiteCelestial : Hover
	{
		public WhiteCelestial() {
			speedY = 1f;
			accelerationY = 0.1f;
		}
		...
		public override void SetDefaults() {
			...
			banner = npc.type;
			bannerItem = ItemType<WhiteCelestialBanner>();
		}
				...
				if (ai == -1f) {
					for (int k = 0; k < 255; k++) {
						...
						if (Math.Abs(npc.Center.X - target.Center.X) < 16f * 30f && Math.Abs(npc.Center.Y - target.Center.Y) < 16f * 20f) {
							target.AddBuff(BuffID.RapidHealing, 240, true);
							target.AddBuff(BuffID.Regeneration, 240, true);
							target.AddBuff(BuffID.Swiftness, 240, true);
							target.AddBuff(BuffID.RapidHealing, 240, true);
							target.AddBuff(BuffID.Calm, 240, true);
		...
		{
			Item.NewItem(npc.getRect(), mod.ItemType("WhiteCelestialShard"), Main.rand.Next(15, 18));
		...
		public override void OnHitPlayer(Player player, int damage, bool crit) {
			if (Main.rand.NextBool(3)) {
				...
				player.AddBuff(BuffID.Regeneration, 240, true);
				player.AddBuff(BuffID.Swiftness, 240, true);
				player.AddBuff(BuffID.ManaRegeneration, 240, true);
				player.AddBuff(BuffID.RapidHealing, 240, true);
				player.AddBuff(BuffID.Calm, 240, true);
			}
		}
