...
{
	public class The404LastPrismBeam : ModProjectile
	{
		// A helpful math constant for performing beam angling calculations.
		private const float PiBeamDivisor = MathHelper.Pi / The404LastPrismHoldout.NumBeams;
		...
		{
			DisplayName.SetDefault("404 Infused Prism Beam");
		}
		...
		public override void AI()
		{
			...
			if (projectile.type != ModContent.ProjectileType<The404LastPrismBeam>() || !hostPrism.active || hostPrism.type != ModContent.ProjectileType<The404LastPrismHoldout>())
			{
				projectile.Kill();
				return;
			}
			...
			float chargeRatio = MathHelper.Clamp(hostPrism.ai[0] / The404LastPrismHoldout.MaxCharge, 0f, 1f);
			...
			projectile.friendly = hostPrism.ai[0] > The404LastPrismHoldout.DamageStart;

			// This offset is used to make each individual beam orient differently based on its Beam ID.
			float beamIdOffset = BeamID - The404LastPrismHoldout.NumBeams / 2f + 0.5f;
			float beamSpread;
			float spinRate;
			...
			float deviationAngle = (hostPrism.ai[0] + beamIdOffset * spinRate) / (spinRate * The404LastPrismHoldout.NumBeams) * MathHelper.TwoPi;
		...
		private Color GetOuterBeamColor()
		{
			...
			float hue = (BeamID / The404LastPrismHoldout.NumBeams) % BeamHueVariance + BeamColorHue;
