...
namespace MoTools.Projectiles
{
	...
	internal class The404NukeClone : ModProjectile
	{
		public override void SetDefaults() {
			...
			float rand = Main.rand.NextFloat(-75, 75);
			while (rand == 0)
			{
				rand = Main.rand.NextFloat(-75, 75);
			}
			if (rand > 0)
			{
				projectile.position.X = projectile.position.X + 150;
			}
			if (rand < 0)
			{
				projectile.position.X = projectile.position.X - 150;
		...
		public override void Kill(int timeLeft) {
			// If we are the original projectile, spawn the 10 child projectiles
			if (projectile.ai[1] == 0) {
				for (int i = 0; i < 10; i++) {
