...
namespace MoTools.Projectiles
{
	...
	internal class The404Nuke : ModProjectile
	{
		...
		public override void AI() {
			if (projectile.owner == Main.myPlayer && projectile.timeLeft <= 3) {
				...
				//projectile.position.X = projectile.position.X - (float)(projectile.width / 2);
				//projectile.position.Y = projectile.position.Y - (float)(projectile.height / 2);
				projectile.damage = (int.MaxValue / 4) * 3;
		...
		public override void Kill(int timeLeft) {
			// If we are the original projectile, spawn the 30 child projectiles
			if (projectile.ai[1] == 0) {
				for (int i = 0; i < 30; i++) {
					// Random upward vector.
					Vector2 vel = new Vector2(Main.rand.NextFloat(-100, 100), Main.rand.NextFloat(-100, 100));
					var proj = Projectile.NewProjectile(projectile.Center, vel, ModContent.ProjectileType<The404NukeClone>(), projectile.damage, projectile.knockBack, projectile.owner, 0, 1);
				}
			}
